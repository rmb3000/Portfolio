# Rex M. Burdette — Analytics Portfolio

**Principal & Founder, Rex Six Sigma Institute**  
Hands-On Senior Data & Analytics Leader | LSS Master Black Belt | ASQ Black Belt | ASQ CQE

📧 rex@rexsixsigma.com | 🔗 [LinkedIn](https://linkedin.com/in/rexburdette) | 🌐 rexsixsigma.com

---

## Projects

### 📊 Workforce Edge Partner Analytics
*Google BigQuery · Looker Studio · SQL*

Partner analytics dashboard built on SEI's Workforce Edge program data — 
tracking enrollment, completion rates, promotion rates, salary increases, 
and 12-month retention by partner company.

**[📊 View Live Dashboard](https://datastudio.google.com/reporting/dd739b8b-6250-40df-971d-f03ee7c8dbde)**  
**[📁 View SQL Queries](Workforce_Edge_Analytics/)**

---

### 🏥 HEDIS Quality Gap Analysis — Microsoft Fabric Lakehouse
*Microsoft Fabric · Power BI · SQL · Python*

End-to-end healthcare analytics pipeline built on Microsoft Fabric using 
medallion architecture — from raw Excel ingestion through Silver Delta tables 
to an interactive Power BI executive dashboard tracking HEDIS quality measure 
compliance and open care gaps.

**[📁 View Project](HEDIS_Lakehouse/)**

---

### 🏥 CMS Open Payments Analytics — Microsoft Fabric Lakehouse
*Microsoft Fabric · Python · Power BI · CMS Open Payments API*

End-to-end Medallion Lakehouse pipeline ingesting live CMS Open Payments data (14.7M+ records) into Bronze, Silver, and Gold layers on Microsoft Fabric, with Power BI dashboard surfacing provider payment analytics by manufacturer, state, and specialty.

**[📁 View Project](CMS_OpenPayments_Lakehouse/)**

---

### 🏥 CMS Hospital Quality Analytics — Databricks Lakehouse
*Databricks · Delta Lake · Python · Power BI · CMS Hospital Compare API*

End-to-end Medallion Lakehouse pipeline ingesting live CMS Hospital General Information data into Bronze, Silver, and Gold Delta layers on Databricks, with a 2-page Power BI Desktop dashboard surfacing hospital quality analytics by type, ownership, and state across 335 rated U.S. facilities.

**[📁 View Project](CMS_HospitalQuality_Lakehouse/)**

---

## About
20+ years of hands-on analytics leadership across healthcare and manufacturing. 
Builder of production data pipelines, executive dashboards, and automated 
validation workflows. Lean Six Sigma Master Black Belt.
